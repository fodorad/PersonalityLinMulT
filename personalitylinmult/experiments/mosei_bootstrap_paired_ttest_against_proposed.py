...
RANDOM_SEED = 42
# Compare MulT vs LinMulT within each feature set
PAIRS = [
    ("OOB_MulT",    "OOB_LinMulT"),
    ("WFR_MulT",    "WFR_LinMulT"),
    ("OOWFR_MulT",  "OOWFR_LinMulT"),
]

# Bonferroni correction for multiple comparisons (3 feature sets)
...
def determine_direction(mean_diff, metric_name, is_significant):
    ...
    Returns: 'Model A', 'Model B', or '-' (no significant difference)
    """
    ...
    if higher_is_better:
        return "Model A" if mean_diff > 0 else "Model B"
    else:
        return "Model B" if mean_diff > 0 else "Model A"
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"BOOTSTRAP PAIRED T-TEST: MulT vs LinMulT (within each feature set)")
    ...
    display_df["Model A"] = display_df["Model A"].apply(format_model_name)
    display_df["Model B"] = display_df["Model B"].apply(format_model_name)
    ...
    # Save to CSV
    output_csv = os.path.join(RESULTS_DIR, f"{DB_NAME}_bootstrap_paired_ttest_summary_against-proposed.csv")
