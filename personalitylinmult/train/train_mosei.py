...
from personalitylinmult.train.mosei import OOWFRDataModule
...
class ModelWrapper(L.LightningModule):
    def __init__(self, model, config: dict):
        ...
        self.total_steps = config.get('total_steps', 
            config.get('n_epochs', 20) * config.get('train_size', 15969) / config.get('batch_size', 8)) # n_epochs * train_size / batch_size
...
def train_model(config: dict):
    ...
    total_steps = n_epochs * config.get('train_size', 15969) / batch_size # 64 batch size: 4990, 16: 19961
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train script for MOSEI dataset and LinMulT OOWFR")
    parser.add_argument("--db_config_path", type=str, default='config/mosei_config.yaml', help="path to the dataset config file")
    parser.add_argument("--model_config_path", type=str, default='config/mosei_OOWFR_LinMulT.yaml', help="path to the model config file")
    parser.add_argument("--train_config_path", type=str, default='config/mosei_train_config.yaml', help="path to the train config file")
