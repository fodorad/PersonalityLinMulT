...
class EgemapsDataset(Dataset):
    ...
    def __init__(self, subset: str, db_root: str):
        ...
        self.samples = self._load_samples(self.db_root / 'cache' / f'mosi_{subset}.pkl')
...
class MosiDataset(Dataset):
    ...
    def __init__(self, subset: str, config: dict | None = None):
        ...
        self.subset = subset
        self.db_root = Path(self.config.get('db_root', 'data/db_processed/mosi'))
        self.samples = self._load_samples(self.db_root / 'cache' / f'mosi_{subset}.pkl')
...
class MosiDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            print('[MOSI] Load train data...')
            self.dataset_train = MosiDataset('train', self.config)
            print('[MOSI] Load valid data...')
            self.dataset_valid = MosiDataset('valid', self.config)

        if stage == "test":
            print('[MOSI] Load test data...')
            self.dataset_test = MosiDataset('test', self.config)
...
if __name__ == "__main__":
    calculate_standardization('data/db_processed/mosi') # calculate and save standardization params