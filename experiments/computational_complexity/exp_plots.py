...
from personalitylinmult.computational_complexity.performance_metrics import speedup_ratio, percentage_reduction
...
def plot_inference_complexity(mult_results, linmult_results, output_dir: str = 'results/computational_complexity'):
    ...
    output_path = Path(output_dir) / "scalability_plots.png"
...
def plot_train_complexity(output_dir: str = 'results/computational_complexity'):
    batch_sizes = [8, 64]
    ...
    plt.tight_layout()
    plt.savefig(Path(output_dir) / 'train_complexity.png')
    plt.close()


def plot_inference_batch(output_dir: str = 'results/computational_complexity'):
    print('\nCreating plots about inference time across batch sizes...')

    with open(Path(output_dir) / 'inference_metrics_gpu.pkl', 'rb') as f:
        gpu_data = pickle.load(f)

    with open(Path(output_dir) / 'inference_metrics_cpu.pkl', 'rb') as f:
    ...
    print('GPU LinMulT inference time mean for batch size 1, 2, 4, 8:', gpu_linmult_mean)
    ...
    print('CPU LinMulT inference time mean for batch size 1, 2, 4, 8:', cpu_linmult_mean)
    ...
    for i in range(len(gpu_mult_mean)):
        speedup_val = f"{gpu_speedup[i]:.1f}x"
    ...
    for i in range(len(cpu_mult_mean)):
        speedup_val = f"{cpu_speedup[i]:.1f}x"
    ...
    axs[1].set_yticks(range(0, 13100, 1000))
    ...
    plt.savefig(Path(output_dir) / "plot_inference_batch.png")
    plt.close()


def plot_inference_window(output_dir: str = 'results/computational_complexity'):
    ...
    print('GPU LinMulT inference time mean for window size 7.5s, 15s, 22.5s, 30s:', gpu_linmult_mean)
    ...
    print('CPU LinMulT inference time mean for window size 7.5s, 15s, 22.5s, 30s:', cpu_linmult_mean)
    ...
    axs[0].set_title('GPU inference time with speedup across window sizes', fontsize=title_size)
    axs[0].legend(loc='upper left', framealpha=1.0)
    axs[0].grid(True)
    ...
    axs[1].set_ylabel('Inference time (ms)', fontsize=axis_label_size)
    axs[1].set_yticks(range(0, 7600, 500))
    axs[1].set_xticks(indices[:len(cpu_linmult_mean)] + (bar_width + space_between_bars)/2)
    ...
    # Adjust layout and show the plot
    plt.tight_layout(pad=2.0)
    plt.savefig(Path(output_dir) / "plot_inference_window.png")
    plt.close()


def plot_memory_comparison(output_dir: str = 'results/computational_complexity'):
    print('\nCreating plots about allocated and reserved GPU memory...')
    ...
    for i in range(len(batch_mult_allocated)):
        ...
        reduction_text = f"{batch_allocated_reduction[i]:.1f}% [{batch_reserved_reduction[i]:.1f}%]"
    ...
    for i in range(len(mult_allocated)):
        ...
        reduction_text = f"{allocated_reduction[i]:.1f}% [{reserved_reduction[i]:.1f}%]"
    ...
    # Adjust layout and show the plot
    plt.tight_layout(pad=2.0)
    plt.savefig(Path(output_dir) / "plot_inference_memory.png")
    plt.close()


def plot_flops_window(output_dir: str = 'results/computational_complexity'):
    print('\nCreating plots about model FLOPs...')

    with open(Path(output_dir) / 'training_flops_gpu.pkl', 'rb') as f:
    ...
    for i in range(len(mult_inference_flops)):
        ...
        reduction_text = f"{inference_flops_reduction[i]:.1f}% [{training_flops_reduction[i]:.1f}%]"
    ...
    # Adjust layout and show the plot
    plt.tight_layout(pad=2.0)
    plt.savefig(Path(output_dir) / "plot_flops.png")
    plt.close()
